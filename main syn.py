...
#%%##########################################################################################
class mapping:
    def __init__(self,action,accosiator_HD_AP):
        self.action=action
    ...
    def _rho_down(self):
        ...
        for k in range(number_subchannel):
            for ap in range(number_AP):
                ...
                self.rho_2=np.sum(self.rho[ap,:,list_u],axis=2)
                inx,iny=np.where(self.rho_2==0)
                ...
                if len(iny)==0:
                    self.temp_rho[ap,k,list_u]=1
                    self.temp_rho_prod = self.temp_rho*self.temp_action_resh
                    self.temp_rho_prod=np.reshape(self.temp_rho_prod[ap,k,:],[number_HD])
                    self.temp_u=np.argmax(self.temp_rho_prod)
                    self.rho[ap,k,self.temp_u]=1
                else:
                    self.temp_u=list_u[iny[0]]
                    self.rho[ap,k,self.temp_u]=1
        cnt_u=0
        done_u=0
        for u in range(number_HD):
    ...
    def _rho_up(self):
        ...
        self.temp_action=(self.action[self.e1:self.e2]+1)/2
        self.temp_action_resh=np.reshape(self.temp_action,[number_AP,number_subchannel,number_HD])
        for k in range(number_subchannel):
            for ap in range(number_AP):
                list_u = np.where(self.accosiator_HD_AP[:,ap]==1)
                self.temp_rho=np.zeros([number_AP,number_subchannel,number_HD])
                self.rho_1=np.sum(self.rho_hat[ap,:,list_u],axis=2)
                inx,iny=np.where(self.rho_1==0)
                if len(iny)==0:
                    self.temp_rho[ap,k,list_u]=1
                    self.temp_rho_prod = self.temp_rho*self.temp_action_resh
                    self.temp_rho_prod=np.reshape(self.temp_rho_prod[ap,k,:],[number_HD])
    ...
    def _power_down(self,rho):
        ...
        self.temp_action_resh=np.reshape(self.temp_action,[number_AP,number_subchannel,number_HD])
        self.temp_action_resh_new=((self.temp_action_resh+1)/2)+.001
        cnt_u=0
        done_u=0
        for u in range(number_HD):
            for ap in range(number_AP):
    ...
    def _power_up(self, rho_hat):
        ...
        cnt_u=0
        done_u=0
        self.mat_energy=np.ones([number_HD])*E_max
        for u in range(number_HD):
            for ap in range(number_AP):
                if self.accosiator_HD_AP[u,ap]==1:
                    for k in range(number_subchannel):
